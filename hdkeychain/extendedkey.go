...
func (k *ExtendedKey) pubKeyBytes() []byte {
	...
	// key if needed.
	if len(k.pubKey) == 0 {
		pkx, pky := bsvec.S256().ScalarBaseMult(k.key)
		pubKey := bsvec.PublicKey{Curve: bsvec.S256(), X: pkx, Y: pky}
...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	if ilNum.Cmp(bsvec.S256().N) >= 0 || ilNum.Sign() == 0 {
	...
	if k.isPrivate {
		...
		ilx, ily := bsvec.S256().ScalarBaseMult(il)
		...
		// public key.
		pubKey, err := bsvec.ParsePubKey(k.key, bsvec.S256())
		if err != nil {
			return nil, err
		}
		...
		//
		// childKey = serP(point(parse256(Il)) + parentKey)
		childX, childY := bsvec.S256().Add(ilx, ily, pubKey.X, pubKey.Y)
		pk := bsvec.PublicKey{Curve: bsvec.S256(), X: childX, Y: childY}
...
// ECPubKey converts the extended key to a bsvec public key and returns it.
func (k *ExtendedKey) ECPubKey() (*bsvec.PublicKey, error) {
	return bsvec.ParsePubKey(k.pubKeyBytes(), bsvec.S256())
}

// ECPrivKey converts the extended key to a bsvec private key and returns it.
...
func (k *ExtendedKey) ECPrivKey() (*bsvec.PrivateKey, error) {
	if !k.isPrivate {
...
func NewMaster(seed []byte, net *chaincfg.Params) (*ExtendedKey, error) {
	...
	if secretKeyNum.Cmp(bsvec.S256().N) >= 0 || secretKeyNum.Sign() == 0 {
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	if isPrivate {
		...
		// secp256k1 curve.
		_, err := bsvec.ParsePubKey(keyData, bsvec.S256())
		if err != nil {
			return nil, err
		}
	}
